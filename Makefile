...
$(BUILD_DIR)/main.c: $(BUILD_DIR)
	cobc -m -F -C -o $@ $(SRC_DIR)/main.cob $(COBOL_CALLED_COBC) $(COBOL_EXPORTS_COBC)

$(BUILD_DIR)/web/main.js: $(BUILD_DIR)/main.c $(BUILD_DIR)/web
	emcc -o $@ $< -s USE_SDL=2 -lgmp -lcob -lcobdom -s EXPORTED_FUNCTIONS=_malloc,_free,_cob_init,$(COBOL_EXPORTS_EMCC) -s EXPORTED_RUNTIME_METHODS=ccall,cwrap,HEAP8 -Wno-deprecated-non-prototype
